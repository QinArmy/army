...
public final class AutoGeneratedKeyGenerator implements PostFieldGenerator {

    private static class Holder {

        private static final AutoGeneratedKeyGenerator INSTANCE = new AutoGeneratedKeyGenerator();

        private static final Map<Class<?>, PostFieldGenerator> AUTO_GENERATOR_FUNCTIONS = createAutoGeneratorFunctions();


        private static Map<Class<?>, PostFieldGenerator> createAutoGeneratorFunctions() {

            Map<Class<?>, PostFieldGenerator> map = new HashMap<>(8);
    ...
    public static AutoGeneratedKeyGenerator build(FieldMeta<?, ?> fieldMeta, Environment env) {
        if (!fieldMeta.primary()
            ...
            throw new MetaException(ErrorCode.META_ERROR, String.format(
                    "entity[%s] prop[%s] type[%s] isn'field supported by %s"
    ...
    public Object apply(FieldMeta<?, ?> fieldMeta, ResultSet resultSet) throws SQLException {
        PostFieldGenerator generator = Holder.AUTO_GENERATOR_FUNCTIONS.get(fieldMeta.javaType());
        Assert.notNull(generator, "paramMeta error");
