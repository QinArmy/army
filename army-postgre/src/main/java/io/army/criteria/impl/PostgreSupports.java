...
abstract class PostgreSupports extends CriteriaSupports {
    ...
    static PgExpressionObjectConsumer pgObjectConsumer(final Consumer<ArmyExpressionElement> consumer,
                                                       final String objectFuncName,
                                                       final MappingType funcReturnType) {
        return new PgExpressionObjectConsumer(consumer, objectFuncName, funcReturnType);
    }
    ...
    static final class PgExpressionObjectConsumer implements PgObjectConsumer, PostgreStatement._PgExpObjectValueClause,
            PostgreStatement._PgExpObjectSpaceClause {
        private final Consumer<ArmyExpressionElement> consumer;
        private final String objectFuncName;

        private final MappingType funcReturnType;

        private ArmyExpression keyExp;

        private Boolean state;

        private PgExpressionObjectConsumer(Consumer<ArmyExpressionElement> consumer,
                                           String objectFuncName, MappingType funcReturnType) {
            assert objectFuncName.equalsIgnoreCase("JSON_BUILD_OBJECT")
                    || objectFuncName.equalsIgnoreCase("JSONB_BUILD_OBJECT");
            this.consumer = consumer;
            this.objectFuncName = objectFuncName;
            this.funcReturnType = funcReturnType;
        }

        @Override
        public PostgreStatement._PgExpObjectValueClause space(final @Nullable String keName) {
            if (this.state != null) {
            ...
            return this.onAddKey(SQLs.literal(NoCastTextType.INSTANCE, keName));
        }

        @Override
        public PostgreStatement._PgExpObjectValueClause space(Expression keyExp) {
            if (this.state != null) {
                throw CriteriaUtils.spaceMethodNotFirst();
            }
            this.state = Boolean.TRUE;
            return this.onAddKey(keyExp);
        }

        @Override
        public PostgreStatement._PgExpObjectValueClause comma(final @Nullable String keName) {
            if (keName == null) {
                throw ContextStack.clearStackAndNullPointer();
            }
            return this.onAddKey(SQLs.literal(NoCastTextType.INSTANCE, keName));
        ...
        @Override
        public PostgreStatement._PgExpObjectValueClause comma(Expression keyExp) {
            return this.onAddKey(keyExp);
        }

        @Override
        public PgObjectConsumer comma(ExpressionElement valueExp) {
            return this.onAddValue(valueExp);
        }

        @Override
        public PgObjectConsumer comma(final String alias, SQLs.SymbolPeriod period,
                                      final TableMeta<?> table) {
            final _SelectionGroup._TableFieldGroup group;
            group = (_SelectionGroup._TableFieldGroup) ContextStack.peek().row(alias, period, table);
            return this.onAddValue(new TableFieldGroupObjectValue(group, this.objectFuncName, this.funcReturnType));
        }
        ...
        public PgObjectConsumer comma(final String alias, SQLs.SymbolPeriod period,
            ...
            group = (DerivedFieldGroup) ContextStack.peek().row(alias, period, asterisk);
            return this.onAddValue(new DerivedGroupObjectValue(group, this.objectFuncName, this.funcReturnType));
        }

        private PostgreStatement._PgExpObjectValueClause onAddKey(final @Nullable Expression keyExp) {
            if (this.state != Boolean.TRUE) {
                throw ContextStack.clearStackAnd(_Exceptions::castCriteriaApi);
            } else if (this.keyExp != null) {
                throw ContextStack.clearStackAnd(_Exceptions::castCriteriaApi);
            } else if (keyExp == null) {
                throw ContextStack.clearStackAndNullPointer();
            } else if (!(keyExp instanceof FunctionArg.SingleFunctionArg)) {
                String m = String.format("don't support %s.", keyExp.getClass().getName());
                throw ContextStack.clearStackAndCriteriaError(m);
            }
            this.keyExp = (ArmyExpression) keyExp;
            return this;
        }

        private PgObjectConsumer onAddValue(final @Nullable ExpressionElement valueExp) {
            final ArmyExpression keyExp = this.keyExp;
            if (keyExp == null) {
                throw ContextStack.clearStackAnd(_Exceptions::castCriteriaApi);
            } else if (valueExp == null) {
                throw ContextStack.clearStackAndNullPointer();
            } else if (!(valueExp instanceof ArmyExpressionElement)) {
                throw ContextStack.clearStackAndNonArmyExpression();
            }
            this.keyExp = null;
            this.consumer.accept(keyExp);
            this.consumer.accept((ArmyExpressionElement) valueExp);
            return this;
        }

        void endConsumer() {
            if (this.keyExp != null) {
                throw ContextStack.clearStackAndCriteriaError("You don't add object value.");
            }
            this.state = Boolean.FALSE;
        }


    }//ExpressionObjectConsumer


    private static final class TableFieldGroupObjectValue extends NonOperationExpression {
        ...
        private TableFieldGroupObjectValue(_SelectionGroup._TableFieldGroup group, String objectFuncName,
    ...
    }//TableFieldGroupObjectValue

    private static final class DerivedGroupObjectValue extends NonOperationExpression {

        private final DerivedFieldGroup group;
        ...
        private DerivedGroupObjectValue(DerivedFieldGroup group, String objectFuncName,
                                        MappingType returnType) {
            this.group = group;
            this.objectFuncName = objectFuncName;
            this.returnType = returnType;
        ...
        public void appendSql(final _SqlContext context) {
            ...
            final String safeDerivedAlias;
            safeDerivedAlias = parser.identifier(this.group.tableAlias());
