 ...
 *         <li>{@link PostgreRangeArrayType}</li>
 *     </ul>
 * </p>
 *
...
abstract class ArmyPostgreRangeType<T> extends _ArmyNoInjectionMapping {
    ...
    ArmyPostgreRangeType(Class<?> javaType, Class<T> elementType, @Nullable RangeFunction<T, ?> rangeFunc,
        ...
        if (javaType == String.class || ArmyPostgreRange.class.isAssignableFrom(javaType)) {
            ...
            this.mockFunction = createMockFunction(javaType, elementType);
        }
    }
    ...
    public static <T, R> RangeFunction<T, R> createRangeFunction(final Class<R> javaType, final Class<T> elementType,
        ...
        if (methodName.endsWith("::new")) {
            ...
            final Method method;
            method = loadFactoryMethod(javaType, methodName, colonIndex, elementType);
            function = (lower, includeLower, upper, includeUpper) -> {
                try {
                    final Object result;
    ...
    static <T, R> R parseRange(final String text, final @Nullable RangeFunction<T, R> rangeFunc,
        ...
        if (rangeFunc == null) {
            ...
            try {
                value = parseNonEmptyRange(text, 0, text.length(), rangeFunc, function);
            } catch (Throwable e) {
    ...
    private static Method loadFactoryMethod(final Class<?> javaType, final String methodName, final int colonIndex,
        ...
        try {
            ...
            final Method method;
            method = Class.forName(methodName.substring(0, colonIndex))
                    .getMethod(methodName.substring(colonIndex + 2),
    ...
    public interface UserDefinedRangeType<T> {
         ...
         */
        PostgreSingleRangeType.MockRangeFunction<T> mockFunction();
