...
class ArmySyncRmSession extends ArmySyncSession implements SyncRmSession {
    ...
    public boolean isRollbackOnly() {
        ...
        final Integer flags;
        return info != null
                && info.valueOf(ArmyOption.XA_STATES) == XaStates.IDLE
                && (flags = info.valueOf(ArmyOption.XA_FLAGS)) != null
                && (flags & TM_FAIL) != 0;
    }
    ...
    public final TransactionInfo end(Xid xid) {
        return this.end(xid, TM_NO_FLAGS, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final TransactionInfo end(Xid xid, int flags) {
        return this.end(xid, flags, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final TransactionInfo end(final @Nullable Xid xid, final int flags, Function<ArmyOption<?>, ?> optionFunc) {
        if (isClosed()) {
            throw _Exceptions.sessionClosed(this);
        } else if (xid == null) {
            // no bug,never here
        ...
        } else if (!(infoXid = lastInfo.nonNullOf(ArmyOption.XID)).equals(xid)) {
        ...
        assert info.inTransaction(); // fail ,executor bug
        assert infoXid.equals(info.valueOf(ArmyOption.XID)); // use infoXid ; fail ,executor bug
    ...
    public final int prepare(Xid xid) {
        return this.prepare(xid, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final int prepare(final @Nullable Xid xid, Function<ArmyOption<?>, ?> optionFunc) {
        if (isClosed()) {
            throw _Exceptions.sessionClosed(this);
        }
        ...
        } else if ((lastInfo.nonNullOf(ArmyOption.XA_FLAGS) & RmSession.TM_FAIL) != 0) {
        ...
        this.rollbackOnly = false;
        if (rollbackOnly || (lastInfo.nonNullOf(ArmyOption.XA_FLAGS) & TM_FAIL) != 0) { // rollback only
    ...
    public final void commit(Xid xid) {
        this.commit(xid, TM_NO_FLAGS, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final void commit(Xid xid, int flags) {
        this.commit(xid, flags, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final void commit(final @Nullable Xid xid, final int flags, Function<ArmyOption<?>, ?> optionFunc) {
        if (isClosed()) {
            throw _Exceptions.sessionClosed(this);
        }
        ...
        if ((flags & TM_ONE_PHASE) != 0) {
            ...
            if (info == null) {
                throw _Exceptions.noTransaction(this);
            } else if (!(infoXid = info.nonNullOf(ArmyOption.XID)).equals(xid)) {
                throw _Exceptions.xaNonCurrentTransaction(xid); // use xid
            } else if ((states = info.nonNullOf(ArmyOption.XA_STATES)) != XaStates.IDLE) {
                throw _Exceptions.xaStatesDontSupportCommitCommand(infoXid, states); // use infoXid
            } else if (this.rollbackOnly || (info.nonNullOf(ArmyOption.XA_FLAGS) & TM_FAIL) != 0) {
                // rollback only
                throw _Exceptions.xaTransactionRollbackOnly(infoXid);
            }
            ...
            // rollback only
            throw _Exceptions.xaTransactionRollbackOnly(info.nonNullOf(ArmyOption.XID));
        } else {
    ...
    public final void rollback(Xid xid) {
        this.rollback(xid, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final void rollback(final @Nullable Xid xid, Function<ArmyOption<?>, ?> optionFunc) {
        if (isClosed()) {
            throw _Exceptions.sessionClosed(this);
        } else if (xid == null) {
            // application developer no bug,never here
    ...
    public final void forget(Xid xid) {
        this.forget(xid, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final void forget(final @Nullable Xid xid, Function<ArmyOption<?>, ?> optionFunc) {
        if (isClosed()) {
            throw _Exceptions.sessionClosed(this);
    ...
    public final List<Xid> recoverList(int flags) {
        return this.recoverList(flags, ArmyOption.EMPTY_OPTION_FUNC);
    }

    @Override
    public final List<Xid> recoverList(int flags, Function<ArmyOption<?>, ?> optionFunc) {
    ...
    public final Stream<Xid> recover(int flags) {
        return this.recover(flags, ArmyOption.EMPTY_OPTION_FUNC, StreamOption.defaultOption());
    }
